...
void bookQuiz();//outputs quiz questions, stores answer from the user in quizTotal
//and outputs a book option based on quizTotal
void file_test(string& bookName, string& newBookName, string& book_description);//function test to see if we can put text into specific file
int ifBook(string bookDescription, int& option);
...
int main()
{
	...
	if (option == 6)
	{
		file_test(bookDescription, bookName, newBookName);
	}
	else 
	{
		file_test(bookDescription, bookName, newBookName);
...
void bookQuiz() //outputs book selection based on quizTotal(quiz answer from the user)
{
...
void file_test(string& bookName, string& newBookName, string& book_description)
{
	srand(time(0));//sets random numbers to output differently each time
	int num = (rand() % 100000);//randomizes purchase code
	//THIS WORKS -----------------------------------------------------------------------
	...
	if (ans == 'n' || ans == 'N')//if user doesn't want to change name
	{
		...
		outputFile_One << "This is your purchase code: " << num << endl
			<< "Thank you for shopping at Visual Studio, have a great day!" << endl;//inputs text into .txt file
	...
	if (ans == 'y' || ans == 'Y')
	{
		...
		{
			cout << "New text file has failed" << endl;
			exit(1);
		}

		outputFile_Two << "This is your purchase code: " << num << endl
			<< "Thank you for shopping at visual studio, have a great day!" << endl;
